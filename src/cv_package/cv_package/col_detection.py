...
class ColourDetect(Node):
    def __init__(self):
        ...
        self.exit_prog = False
        self.fig.canvas.mpl_connect('key_press_event', self.on_key)
        self.identified_colours = {'yellow': False, 'red' : False}
    ...
    def callback_listener(self, msg):
        ...
        self.detect_and_publish(frame, mask_red, 'yellow', (0,255,255))
        self.detect_and_publish(frame, mask_red, 'red', (0,0,255))
    ...
    def det_publish(self, frame, mask, name_of_colour, colour_bgr):
        #make contours to track red
        ...
        for cont in conts:
            ...
            if area > 300:
                ...
                if not self.identified_colours[name_of_colour]:
                    self.publish_marker(x,y,w,h,name_of_colour, colour_bgr)
                    self.identified_colours[name_of_colour] = True
                    self.publisher.publish(String(data = name_of_colour))
                    self.get_logger().info(f"{name_of_colour} detected")

    def maker_publish(self, x,y,w,h, name_of_colour):
        marker = Marker()
        ...
        marker.scale.y = h
        marker.scale.z = 0.1
        marker.color.a = 1.0  #for full opacity
        
        if name_of_colour == 'yellow':
            marker.color.r = 1.0
            marker.color.g = 1.0
            marker.color.b = 0.0
            
        elif name_of_colour == 'red':
            marker.color.r = 1.0
            marker.color.g = 0.0
            marker.color.b = 0.0
