...
class OCRNode(Node):
    ...
    def listener_callback(self, msg):
        ...
        if contours:
            ...
            for contour in filtered_contours:
                ...
                if 0.5 < aspect_ratio < 2.0:
                    ...
                    if results:
                        recognised_digits.extend([r for r in results if r.isdigit() and 0 <= int(r) <= 9])
                        for digit in recognised_digits:
                            self.get_logger().info(f"Recognized digit: {digit}")
        ...
        if recognised_digits:
            ...
            if self.stable_prediction != self.last_printed_prediction:
                self.get_logger().info(f"CURRENT_PREDICTION: {self.stable_prediction} CONFIDENCE: {confidence:.2f}")
    ...
    def destroy_node(self):
        ...
        plt.close()
        super().destroy_node()
        self.get_logger().info(f"FINAL PREDICTIONS: {self.all_predictions}")
